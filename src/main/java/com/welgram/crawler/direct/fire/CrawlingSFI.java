...
public abstract class CrawlingSFI extends SeleniumCrawler implements Scrapable {
	...
	protected void setTreaties(CrawlingProduct info) throws Exception {
		...
		if (targetTreatyList.size() < myTreatyList.size()) {
			...
			try {
				throw new Exception("웹에 특약보다 상품마스터의 특약이 더 많습니다." + "\n개수 : " + myTreatyList.size() + "개 -- "+"특약 목록 : " + saveTreaty);
	...
	protected void btnCalc() throws Exception {
		...
		// 로딩이 끝날 때까지 기다림
		try {
			wait.until(ExpectedConditions.invisibilityOf(helper.waitVisibilityOfElementLocated(By.cssSelector("div.ui-loading-wrap"))));
	...
	protected List<CrawlingTreaty> getPageTreatyListFromTable(CrawlingProduct info, Boolean customizedPlan) throws Exception {
		...
		for (int i = 0; i < pageTr.size(); i++) {  // <원수사 treaty loop>
			...
			WebElement tr = helper.waitPresenceOfElementLocated(By.cssSelector("tbody[id^='coverage']")).findElements(By.cssSelector("tr")).get(i);
...
//		return pagePlanTreatyList;
	...
	protected List<CrawlingTreaty> getPageTreatyListFromModal(CrawlingProduct info) throws CommonCrawlerException, InterruptedException {

		boolean DTL = info.getProductCode().contains("DTL");

		// ##### 사용하는 메서드들 #####
		Function<WebElement, String> getPageTreatyNameFromTr = (tr) -> getRefinedPageTreatyName(tr.findElement(By.cssSelector("td:nth-child(1)")).getText());
		...
		for (int i = 1 ; i <= trCount; i++) {
			...
			if (tdType.contains("TITLE")) {
				...
				if (info.getProductCode().contains("CHL") && (pageTreatyName.contains("치아") || pageTreatyName.contains("예약"))) {
					break;
				}
...
//	@NotNull
	private PlanState getPlanState(
		CrawlingProduct info, String trCssSelector, String pageTreatyName, WebElement _4thTd
...
//		return pagePlanTreatyList;
	...
	public void setGenderNew(Object obj) throws SetGenderException {
		...
		try {
			...
			Optional<WebElement> genderLabel
				= helper.waitVisibilityOfAllElementsLocatedBy(By.cssSelector("label[for^='gender']"))
					.stream()
					.filter(label -> label.getText().contains(infoGender))
					.findFirst();
	...
	public void setJobNew(Object obj) throws SetJobException {
		...
		try {
			...
			if (info.productCode.equals("SFI_DSS_D029")) { // 무배당 삼성화재 다이렉트 Smart 맞춤보장보험
	...
	public void setInsTermNew(Object obj) throws SetInsTermException {
		...
		try {
			...
			// 없으면 throw
			matched.orElseThrow(() -> new SetInsTermException(insTerm + "는 선택항목에 없습니다."));
	...
	protected void waitForLoading(By... by) throws InterruptedException {
		...
		WebDriverWait waitForSFI = new WebDriverWait(driver, Duration.ofSeconds(60));
		logger.info("CrawlingSFI.waitForLoading");

		waitForSFI.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("[(class*='loading') || (class*='Loading) || (id*='loading') || (id*='Loading')]")));
//		waitForSFI.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("*[class*='Loading']")));
//		waitForSFI.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("*[id*='loading']")));
//		waitForSFI.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("*[id*='Loading']")));
	...
	public void setNapTermNew(Object obj) throws SetNapTermException {
		...
		try {
			...
			try {
				// 선택가능한 납입기간
				WebElement ddEl = helper.waitPesenceOfAllElementsLocatedBy(By.cssSelector("dd.btn-group"))
					.stream()
			...
			// 없으면 throw
			matched.orElseThrow(() -> new SetNapTermException(napTerm + "이 선택항목에 없습니다."));
	...
	public void setRenewTypeNew(Object obj) throws SetRenewTypeException {
		...
		try {
			...
			try {
				// 선택가능한 갱신형태
				WebElement ddEl
					= helper.waitPesenceOfAllElementsLocatedBy(By.cssSelector("dd.btn-group"))
						.stream()
			...
			// 없으면 throw
			matched.orElseThrow(() -> new SetRenewTypeException(renewType + "이 선택항목에 없습니다."));
	...
	protected void setSubPlan(CrawlingProduct info) throws CommonCrawlerException, InterruptedException {
		...
		String subStr
			= planSubName.substring(
				0,
				planSubName.contains("플랜")
					? planSubName.indexOf("플랜")
					: planSubName.length()
			).trim();

		// 선택 가능한 플랜
		List<WebElement> list = helper.waitPesenceOfAllElementsLocatedBy(By.cssSelector("thead#coverage-header label"));

		// 일치하는 플랜
		Optional<WebElement> matchedOpt = list.stream().filter( el -> el.getText().contains(subStr)).findFirst();

		// 없으면 throw
		matchedOpt.orElseThrow(() -> new CommonCrawlerException(info.planSubName + "이 선택항목에 없습니다."));
