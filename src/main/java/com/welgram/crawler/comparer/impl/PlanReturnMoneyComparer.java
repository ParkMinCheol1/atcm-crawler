...
//                    .replaceAll("[^0-9.]", "");   //원수사 평균해약환급률
                ...
                if (!ObjectUtils.isEmpty(origin)) {
                    String originTerm
                        = origin.getTerm().replaceAll(" ", "");                                  //기존 납입기간
                    String originPremiumSum
                        = origin.getPremiumSum()
                            .replaceAll("[^0-9]", "");                 //기존 합계보험료
                    String originReturnMoney
                        = origin.getReturnMoney()
                            .replaceAll("[^0-9]", "");               //기존 해약환급금
                    String originReturnRate
                        = origin.getReturnRate()
                            .trim()
                            .replaceAll("[^0-9.]", "");         //기존 해약환급률
                    String originReturnMoneyMin
                        = origin.getReturnMoneyMin()
                            .replaceAll("[^0-9]", "");         //기존 최저해약환급금
                    String originReturnRateMin
                        = origin.getReturnRateMin()
                            .trim()
