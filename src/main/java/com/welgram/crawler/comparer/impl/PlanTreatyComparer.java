...
public class PlanTreatyComparer implements PlanComparer {
    ...
    public boolean compare() {
        ...
        List<String> crawlingTreatNames
            = currentTreatyList.stream()
                .map(t -> t.treatyName)
    ...
    public boolean comparePlanComposition(){
        ...
        for (CrawlingTreaty newTreaty : currentTreatyList) {
            ...
            if (!hasMatchedTreaty) {
                treatiesToAdd.add(new TreatyDiffInfo(info.planId, newTreatyName, newAssureMoney));
            }
    ...
    public String getInfo(PlanCompareResult planCompareResult) {
        ...
        if (treatiesToDelete.size() != 0) {
            ...
            for (TreatyDiffInfo treatyDiffInfo : treatiesToDelete) {
                errorTxt
                    .append("productMasterId : ").append(treatyDiffInfo.getProductMasterId())
        ...
        if (treatiesToUpdateAssureMoney.size() != 0) {
            errorTxt
                .append(" ::: 가입금액 변경해야할 특약 - 원수사와 가입금액이 일치하지 않는 특약 ::: ")
            ...
            for (TreatyDiffInfo a : treatiesToUpdateAssureMoney) {
                ...
                errorTxt
                    .append("productMasterId : ").append(a.getProductMasterId())
                    .append(System.lineSeparator())
        ...
        if (treatiesToAdd.size() != 0) {
            errorTxt
                .append(" ::: 가설에 등록해야할 특약 - 원수사페이지에만 특약명이 존재 ::: ")
            ...
            for (TreatyDiffInfo d : treatiesToAdd) {
                errorTxt
                    .append("보장명 : ").append(d.getPageName()).append(" : ")
