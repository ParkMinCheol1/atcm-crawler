...
import com.welgram.atcm.enums.AnnuityType;
import com.welgram.atcm.enums.Category;
import com.welgram.atcm.enums.ErrorCode;
import com.welgram.atcm.enums.Gender;
import com.welgram.atcm.enums.InsType;
import com.welgram.atcm.enums.PayCycle;
import com.welgram.atcm.enums.PayType;
import com.welgram.atcm.enums.TreatyType;
import com.welgram.atcm.strategy.AtcmTreatyEqualStrategy1;
import com.welgram.atcm.util.Birthday;
import com.welgram.atcm.util.MoneyUtil;
import com.welgram.atcm.util.WaitUtil;
import com.welgram.atcm.vo.AtcmCrawlingInfo;
import com.welgram.atcm.vo.AtcmPlanVO;
import com.welgram.atcm.vo.AtcmPremiumVO;
import com.welgram.atcm.vo.AtcmReturnVO;
import com.welgram.atcm.vo.AtcmReturnIdx;
...
public class GeneralScriptSLI extends GeneralCrawler {
    ...
    @Override
    protected AtcmCrawlingInfo runScript(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {

        Category category = Category.fromAnalysisCode(atcmCrawlingInfo.getCategory());
        boolean isAnnuityCategory = (category == Category.ANT || category == Category.ASV);
        AtcmPlanVO plan = atcmCrawlingInfo.getAtcmPlanVO();

        logger.info("공시실 상품명 찾기");
        findProductName(atcmCrawlingInfo.getProductName());

        logger.info("[group 1] : 가입대상 설정");
        setJoinTargetGroup(atcmCrawlingInfo);

        logger.info("[group 2] : 고객정보 설정");
        setUserInfoGroup(atcmCrawlingInfo);

        logger.info("[group 3] : 가입조건 설정");
        setJoinConditionGroup(atcmCrawlingInfo);

        logger.info("[group 4] : 주계약조건 설정");
        setMainTreatyConditionGroup(atcmCrawlingInfo);

        logger.info("[group 5] : 특약 설정");
        List<AtcmTreatyVO> subTreatyList = atcmCrawlingInfo.getAtcmPlanVO().getAtcmTreatyVOList().stream()
        ...
        logger.info("보험료 크롤링");
        crawlPremiums(atcmCrawlingInfo);

        if(isAnnuityCategory) {
            logger.info("연금수령액 크롤링");
            crawlAnnuityPremiums(atcmCrawlingInfo);
        }

        logger.info("해약환급금 크롤링");
        crawlPlanReturnMoneys(atcmCrawlingInfo);
    ...
    private void crawlAnnuityPremiums(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {

        AtcmPlanVO plan = atcmCrawlingInfo.getAtcmPlanVO();
        AtcmPremiumVO atcmPremiumVO = plan.getAtcmPremiumVO();
        ...
        try {
            ...
            atcmPremiumVO.setAnnuityPremium(new BigInteger(annuityPremium));
    ...
    //가입대상 그룹 설정
    private void setJoinTargetGroup(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
...
//             */
//            if(atcmCrawlingProduct.getJoinTarget() != null) {
//
...
//
//                    logger.info("구분 설정하기");
//                    setJoinTargetContent($contentValueSection, atcmCrawlingProduct.getJoinTarget());
//                }
//            }
        }
    }
    ...
    private void setBirthAndGender(WebElement $section, AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
        ...
        if(isExist) {
            ...
            setBirth($birthValueSection, atcmCrawlingInfo.getBirthday());
            ...
            setGender($genderValueSection, atcmCrawlingInfo.getGender());
        }
        ...
        if (isExist) {
            try {
                ...
                $birthTitleSection = $section.findElement(birthPosition);
                $birthValueSection = $birthTitleSection.findElement(By.xpath("./following-sibling::td[1]"));
                setBirth($birthValueSection, atcmCrawlingInfo.getBabyBirthday());
            } catch (Exception e) {
            ...
            try {
                ...
                setGender($genderValueSection, atcmCrawlingInfo.getBabyGender());
            } catch (Exception e) {
        ...
        if (isExist) {
            ...
            if(atcmCrawlingInfo.getGender() == Gender.FEMALE) {
    ...
    //고객정보 그룹 설정
    private void setUserInfoGroup(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
        ...
        if (isExistGroup) {
            ...
            setBirthAndGender($groupSection, atcmCrawlingInfo);
            ...
            if (isExistContent) {
                ...
                logger.info("직업 설정");
                setJob($contentValueSection, atcmCrawlingInfo.getJob());
            }
        }
    ...
    //가입조건 그룹 설정
    private void setJoinConditionGroup(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
        ...
        AtcmPlanVO plan = atcmCrawlingInfo.getAtcmPlanVO();
        AtcmTreatyVO mainTreaty = plan.getMainTreaty();
        ...
        if (isExistGroup) {
            ...
            if (isExistContent) {
                ...
                logger.info("고액계약 설정");
                setLargeAmountContract($contentValueSection, atcmCrawlingInfo.getLargeAmountContract());
            }
            ...
            if (isExistContent) {
                ...
                setHealthType($contentValueSection, atcmCrawlingInfo.getHealthType());
            }
            ...
            if (isExistContent) {
                ...
                setMedicalBeneficiary($contentValueSection, atcmCrawlingInfo.getMedicalBeneficiary());
            }
            ...
            if (isExistContent) {
                ...
                logger.info("임신주수 설정");
                setPregnancyWeek($contentValueSection, atcmCrawlingInfo.getPregnancyWeek());
            }
...
//                }
    ...
    private void setSubTreatyList(List<AtcmTreatyVO> welgramSubTreatyList) throws Exception {

        try {
            //가입설계에 선택특약이 1개 이상인 경우
            if (welgramSubTreatyList.size() > 0) {
                logger.info("가입설계 특약을 바탕으로 원수사에 세팅하기");
                for (AtcmTreatyVO welgramSubTreaty : welgramSubTreatyList) {
                ...
                List<AtcmTreatyVO> targetTreatyList = new ArrayList<>();
                for (WebElement $treatyTr : $treatyTrList) {
                    AtcmTreatyVO targetTreaty = getTreatyInfoFromTr($treatyTr);
                ...
                boolean result = advancedCompareTreaties(targetTreatyList, welgramSubTreatyList, new AtcmTreatyEqualStrategy1());
                if (result) {
    ...
    private void setTreatyInfoFromTr(WebElement $tr, AtcmTreatyVO subTreaty) throws Exception {
    ...
    private AtcmTreatyVO getTreatyInfoFromTr(WebElement $tr) throws Exception {

        AtcmTreatyVO treaty = null;
    ...
    private void crawlPremiums(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
        ...
        try {
            ...
            if (category == Category.BAB) {
                atcmPremiumVO.setPreBirthPremium(Integer.parseInt(premium));
                atcmPremiumVO.setAfterBirthPremium(Integer.parseInt(afterBirthPremium));
            } else {
                atcmPremiumVO.setPremium(Integer.parseInt(premium));
            }
...
//            }
    ...
    private AtcmReturnIdx getPlanReturnMoneyIdx() throws Exception {

        AtcmReturnIdx atcmReturnIdx = new AtcmReturnIdx();
        ...
        switch(colSize) {
            case 4:
                atcmReturnIdx.setPremiumSumIdx(1);
                atcmReturnIdx.setReturnPremiumIdx(2);
                atcmReturnIdx.setReturnRateIdx(3);
                break;

            case 8:
                atcmReturnIdx.setPremiumSumIdx(1);
                atcmReturnIdx.setReturnPremiumMinIdx(2);
                atcmReturnIdx.setReturnRateMinIdx(3);
                atcmReturnIdx.setReturnPremiumAvgIdx(4);
                atcmReturnIdx.setReturnRateAvgIdx(5);
                atcmReturnIdx.setReturnPremiumIdx(6);
                atcmReturnIdx.setReturnRateIdx(7);
                break;

            case 11:
                atcmReturnIdx.setPremiumSumIdx(1);
                atcmReturnIdx.setReturnPremiumMinIdx(3);
                atcmReturnIdx.setReturnRateMinIdx(4);
                atcmReturnIdx.setReturnPremiumAvgIdx(6);
                atcmReturnIdx.setReturnRateAvgIdx(7);
                atcmReturnIdx.setReturnPremiumIdx(9);
                atcmReturnIdx.setReturnRateIdx(10);
                break;
        ...
        return atcmReturnIdx;
    }



    private void crawlPlanReturnMoneys(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {

        AtcmPlanVO plan = atcmCrawlingInfo.getAtcmPlanVO();
        AtcmPremiumVO atcmPremiumVO = plan.getAtcmPremiumVO();
        List<AtcmReturnVO> atcmReturnVOList = plan.getAtcmReturnVOList();
        ...
        try {
            ...
            AtcmReturnIdx atcmReturnIdx = getPlanReturnMoneyIdx();
            ...
            for (WebElement $tr : $trList) {
                ...
                String premiumSum = $tdList.get(atcmReturnIdx.getPremiumSumIdx()).getText();
                String returnPremium = $tdList.get(atcmReturnIdx.getReturnPremiumIdx()).getText();
                String returnRate = $tdList.get(atcmReturnIdx.getReturnRateIdx()).getText();

                //원수사에서 최저환급금, 평균환급금 정보는 제공할 수도 있고 아닐 수도 있음.
                String returnPremiumMin
                    = (atcmReturnIdx.getReturnPremiumMinIdx() == null)
                        ? null
                        : $tdList.get(atcmReturnIdx.getReturnPremiumMinIdx()).getText();
                String returnRateMin
                    = (atcmReturnIdx.getReturnRateMinIdx() == null)
                        ? null
                        : $tdList.get(atcmReturnIdx.getReturnRateMinIdx()).getText();
                String returnPremiumAvg
                    = (atcmReturnIdx.getReturnPremiumAvgIdx() == null)
                        ? null
                        : $tdList.get(atcmReturnIdx.getReturnPremiumAvgIdx()).getText();
                String returnRateAvg
                    = (atcmReturnIdx.getReturnRateAvgIdx() == null)
                        ? null
                        : $tdList.get(atcmReturnIdx.getReturnRateAvgIdx()).getText();
                ...
                AtcmReturnVO atcmReturnVO = new AtcmReturnVO();
                atcmReturnVO.setTerm(term);
                atcmReturnVO.setPremiumSum(new BigInteger(premiumSum));
                atcmReturnVO.setReturnPremium(new BigInteger(returnPremium));
                atcmReturnVO.setReturnRate(returnRate);
                atcmReturnVO.setReturnPremiumMin(
                    StringUtils.isNotEmpty(returnPremiumMin)
                        ? new BigInteger(returnPremiumMin)
                        : null
                );
                atcmReturnVO.setReturnRateMin(returnRateMin);
                atcmReturnVO.setReturnPremiumAvg(
                    StringUtils.isNotEmpty(returnPremiumAvg)
                        ? new BigInteger(returnPremiumAvg)
                        : null
                );
                atcmReturnVO.setReturnRateAvg(returnRateAvg);

                atcmReturnVOList.add(atcmReturnVO);
            }
            logger.info("해약환급금 크롤링 끝");

            BigInteger expirePremium = atcmReturnVOList.get(atcmReturnVOList.size() - 1).getReturnPremium();
            atcmPremiumVO.setReturnPremium(expirePremium);
    ...
    //주보험조건 그룹 설정
    private void setMainTreatyConditionGroup(AtcmCrawlingInfo atcmCrawlingInfo) throws Exception {
        ...
        if (isExistGroup) {
            ...
            if (isExistContent) {
                ...
                setLifeDesignCostAge($contentValueSection, String.valueOf(atcmCrawlingInfo.getLifeDesignCostAge()));
            }
            ...
            if (isExistContent) {
                ...
                setLifeDesignCostPeriod($contentValueSection, atcmCrawlingInfo.getLifeDesignCostPeriod());
            }
        }
    }
